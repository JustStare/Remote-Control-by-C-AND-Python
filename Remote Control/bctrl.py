...
def ctrl(conn):
    ...
    def Op(key, op, ox, oy):
        ...
        if key == 4:
            ...
            k = keycodeMapping.get(key)  # key的转化 也就是键盘输入值
            if k is not None:
                if op == 100:
                    ag.keyDown(k)       ## 按下key
                elif op == 117:
...
while True:
    ...
    ## 使用线程池
    threading.Thread(target=handle, args=(conn,)).start()  ## 开启视频传输线程
    threading.Thread(target=ctrl, args=(conn,)).start()    ## 开启鼠标，键盘 传输 线程
    threading.Thread(target=server_C.start_run).start()   ## 开启通信
